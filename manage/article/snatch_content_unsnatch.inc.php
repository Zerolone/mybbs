...
* 修改日期：
* 文 件 名：/manage/article/snatch_content_unsnatch.inc.php
* 作    者：Zerolone
*/
...
*/		
function getContentContent ( $Url, $id, $ContentStartFlag, $ContentEndFlag, $FlagAdStart, $FlagAdEnd, $FlagSingle, $ImagePath, $ImageUrl, $utf8)
{
	//获取源数据
...
//	preg_match_all( "/src=(\"|')(.*?)(\"|')/i", DeCodeStr($Content), $temp );
	preg_match_all( "/<img[^>]*src=(\"|')(.*?)(\"|')/i", DeCodeStr($Content), $temp );
...
$id	= $_GET["id"];
//--------------------0---------1----------2-----------------------3--------------------4------------------5----------------6------------7-----------------8
$SqlStr	= 'Select  `a`.`id`, `a`.`url`, `b`.`flagcontentstart`, `b`.`flagcontentend`,`b`.`flagadstart`, `b`.`flagadend`, `a`.`title`, `b`.`flagsingle`, `b`.`utf8`  From `'.table_pre.'article` a, `'.table_pre.'article_source` b where a.sourceid=b.id and `a`.`id`=' . $id;
...
while($DB_Record = nqfetch($temp_query))
{
	...
	$CatchOk	= getContentContent($Url, $id, $ContentStartFlag, $ContentEndFlag, $FlagAdStart, $FlagAdEnd, $FlagSingle, $ImagePath, $ImageUrl, $utf8);
}
?>
