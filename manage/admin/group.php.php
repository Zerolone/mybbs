...
function goto(pagenum){
  ...
  <tr bgcolor="#6A6A6A">
    <td colspan="10" height="26"><b><font color="#FFFFFF">&nbsp;用户管理 &gt;&gt; 用户组查看</font></b></td>
  </tr>
    <tr valign=center align=middle>
      <td width="60" bgcolor=#999999 height=20><p><font color="#FFFFFF">顺序</font></p></td>
      <td width="*" bgcolor=#999999><font color="#FFFFFF">组名</font></td>
  ...
  <form name="form1" action="group_update.php" method="post">
		<?php
			if(isset($group_list))
			{
				...
				{
		?>
		<label for="del<?=$group['id']?>">
    <tr>
      <td><input type="hidden" name="id[]" value="<?=$group['id']?>"><select name="order[]" class="inputbox"><?=getOrderList($group['order'])?></select></td>
      <td>&nbsp;<?=$group['title']?></td>
			<td><input name="del[]" type="checkbox" id="del<?=$group['id']?>" value="<?=$group['id']?>">删除</td>
      <td>&nbsp;<a href="group_edit.php?id=<?=$group['id']?>">修改</a></td>
    </tr>
		</label>
    <?php
				}
