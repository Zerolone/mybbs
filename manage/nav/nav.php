...
//WHERE语句
$SqlstrWhere='';
$SqlstrWhere.= ' WHERE `cateid`='.$cateid;
$SqlstrWhere.= ' AND `userid`='.$userid;

$SqlStr = 'Select count(*) from `'.DB_TABLE_PRE.'nav`';
$SqlStr.=$SqlstrWhere;
...
//------------------0------1--------2---------3----------4-------5-------6
$SqlStr	= 'SELECT `id`, `title`, `order`, `content`, `active`, `url`, `level` FROM `'.DB_TABLE_PRE.'nav`';
$SqlStr.=$SqlstrWhere;
