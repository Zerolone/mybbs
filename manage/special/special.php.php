...
function goto(pagenum){
  ...
  <table width="100%" border="0" align="center" cellpadding="1" cellspacing="1" bgcolor="#CCCCCC" id="DBList">
    <tr valign=center align=middle>
      ...
      <td width="60" bgcolor=#999999><font color="#FFFFFF">查看模块</font></td>
      <td width="40" bgcolor=#999999><font color="#FFFFFF">生成</font></td>
  ...
  <form name="form1" action="update.php" method="post">
    ...
    <tr>
      <td>
			...
			<select name="order[]" class="inputbox"><?=getOrderList($special['order'])?></select>
			</td>
      <td>&nbsp;<?=$special['title']?></td>
      <td>&nbsp;<a href="module.php?parentid=<?=$special['id']?>">下级模块</a></td>
      <td>&nbsp;<a href="update.php?id=<?=$special['id']?>&mode=html">生成</a></td>
      <td>&nbsp;<a href="edit.php?id=<?=$special['id']?>">修改</a></td>
			<td><input name="del[]" type="checkbox" id="del<?=$special['id']?>" value="<?=$special['id']?>"><label for="del<?=$special['id']?>">删除</label></td>
    </tr>
    <?php
				}
		}	
