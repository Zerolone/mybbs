...
?>
<br />
<table cellspacing=1 cellpadding=5 width="98%" bgcolor=#E7E3E7 align=center>
  <tr>
  ...
  <tr>
    ...
    <td class='f_one' align=left onMouseOver="this.className='f_two'" onMouseOut="this.className='f_one'">
    	<a href='<?=$forum['urltopic']?>' TARGET=_blank><?=$forum['logo']?></a>&nbsp;
      <a href='<?=$forum['urltopic']?>' class='fnamecolor'><b><?=$forum['title']?></b></a><br><span class=smalltxt><?=$forum['content']?></span>
    </td>
    <td align=center class=f_two><b><?=$forum['topic']?></b><br /><?=$forum['post']?></td>
    <td class='smalltxt f_one'> 主题: <a href="<?=$forum['urlread']?>"><?=$forum['t']?></a><br>作者: <?=$forum['author']?><br>时间: <?=$forum['lastpost']?> </td>
...
<table cellpadding=5 cellspacing=1 width="98%" bgcolor=#E7E3E7 align=center>
  ...
  <tr height=40>
    ...
    您的等级:<b><?=$group['title']?></b> <?=CREDIT_RVRC?>: <b><?=(int)($user->rvrc/10) . CREDIT_RVRC_EXT?></b> / <?=CREDIT_MONEY?>: <b><?=$user->money . CREDIT_MONEY_EXT?></b> / 
      ...
      您上次登录时间:<?=get_date($user->lastpost)?> | 登录IP:<?=$user->ip?> | <a href="search.php?authorid=<?=$user->uid?>"><b>查看我的主题</b></a>
     <?php }?>
      </td>
  </tr>
</table>